...
def extract_referenced_features(content):
    ...
    # Look for feature names in various formats
    # Markdown links: [feature-name](...)
    link_pattern = re.compile(r"\[([^\]]+)\]\([^\)]*\)")
    for match in link_pattern.finditer(content):
        features.add(match.group(1).strip().lower())

    # Backtick-wrapped names: `feature-name`
    bt_pattern = re.compile(r"`([^`]+)`")
    for match in bt_pattern.finditer(content):
        name = match.group(1).strip().lower()
        if "/" not in name and len(name) < 50:
            features.add(name)

    # Bullet list items that look like feature names
